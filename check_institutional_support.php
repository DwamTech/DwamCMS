...
foreach ($dummyFiles as $file) {
    if (str_ends_with($file, '.jpg')) file_put_contents($file, $jpegHeader);
    else file_put_contents($file, $pdfHeader);
}
...
foreach ($dummyFiles as $file) if (file_exists($file)) unlink($file);
